...
const milestones = [
  { year: "2018", key: "started" },
  { year: "2020", key: "online" },
  { year: "2022", key: "method" },
  { year: "2024", key: "platform" },
];

const valueKeys = ["passion", "conversations", "personalized", "fun"] as const;
...
const statKeys = ["students", "lessons", "rating", "years"] as const;
    ...
    <MarketingLayout>
          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            ...
            >
              ...
              <motion.div
                ...
                className="absolute -bottom-6 -start-6 bg-card border border-border rounded-2xl p-6 shadow-xl"
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <div className="text-2xl font-bold text-foreground">{t("stats.rating")}</div>
              ...
              <motion.div
                ...
                className="absolute top-6 -end-4 bg-primary text-primary-foreground rounded-full px-4 py-2 text-sm font-semibold shadow-lg"
              >
                {t("nativeBadge")} 🇮🇱
              </motion.div>
            </motion.div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {statKeys.map((key, index) => (
              ...
              >
                ...
                <div className="text-muted-foreground">{t(`stats.${key}Label`)}</div>
              </motion.div>
            ))}
          </div>
        </div>
          ...
          <div className="grid md:grid-cols-2 gap-8">
            {valueKeys.map((key, index) => (
              ...
              >
                ...
                <h3 className="text-xl font-bold mb-3 text-foreground">{t(`values.${key}.title`)}</h3>
                <p className="text-muted-foreground leading-relaxed">{t(`values.${key}.description`)}</p>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="relative py-24 bg-muted/30">
        <div className="max-w-4xl mx-auto px-6">
          ...
          >
            <h2 className="text-4xl md:text-5xl font-bold mb-6 text-foreground">
              {t("journey.title")} <span className="text-primary">{t("journey.highlight")}</span>
            </h2>
            <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
              {t("journey.subtitle")}
            </p>
          ...
          <div className="relative">
            {/* Timeline line */}
            <div className="absolute start-8 md:start-1/2 top-0 bottom-0 w-px bg-border md:-translate-x-px" />
            ...
            {milestones.map((milestone, index) => (
              ...
              >
                {/* Timeline dot */}
                <div className="absolute start-8 md:start-1/2 w-4 h-4 rounded-full bg-primary border-4 border-background -translate-x-1/2 mt-2" />

                {/* Content */}
                <div className={`flex-1 ms-16 md:ms-0 ${index % 2 === 0 ? "md:pe-16 md:text-end" : "md:ps-16"}`}>
                  <div className="inline-block px-3 py-1 rounded-full bg-primary/10 text-primary text-sm font-semibold mb-3">
                    {milestone.year}
                  </div>
                  <h3 className="text-xl font-bold mb-2 text-foreground">{t(`milestones.${milestone.key}.title`)}</h3>
                  <p className="text-muted-foreground">{t(`milestones.${milestone.key}.description`)}</p>
                </div>
